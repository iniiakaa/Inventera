<x-layouts.admin active="branches" title="Tambah Cabang - Inventera">
    <header class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8 mt-8 md:mt-0">
        <div class="flex items-center space-x-4">
            ...
            <div>
                ...
                <h2 class="font-display text-headline-lg text-on-surface">Tambah Cabang</h2>
            </div>
        </div>
    </header>
    ...
    @if(Auth::user()->role === 'owner')
        <section class="liquid-glass rounded-xl p-8 max-w-3xl">
            <form action="{{ route('branches.store') }}" method="POST" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" id="code" name="code" value="{{ old('code') }}" required
                               ...
                               placeholder="Mis: JKT-02">
                        @error('code') <p class="text-error font-body-sm mt-1">{{ $message }}</p> @enderror
                    </div>
                    ...
                    <div>
                        ...
                        @error('name') <p class="text-error font-body-sm mt-1">{{ $message }}</p> @enderror
                    </div>
                    ...
                    <div>
                        ...
                        @error('city') <p class="text-error font-body-sm mt-1">{{ $message }}</p> @enderror
                    </div>
                    ...
                    <div>
                        ...
                        @error('manager_name') <p class="text-error font-body-sm mt-1">{{ $message }}</p> @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" id="phone" name="phone" value="{{ old('phone') }}"
                               class="w-full bg-surface/50 border border-outline-variant focus:border-primary focus:ring focus:ring-primary/20 rounded-lg px-4 py-2.5 text-on-surface transition-all placeholder:text-on-surface-variant/50"
                               placeholder="Opsional">
                        @error('phone') <p class="text-error font-body-sm mt-1">{{ $message }}</p> @enderror
                    </div>
                ...
                <div class="pt-6 border-t border-outline-variant/30 mt-6">
                    <h3 class="font-label-lg text-on-surface mb-4">Data Akun Login Manajer</h3>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label for="manager_email" class="block font-label-md text-on-surface mb-2">Email Manajer <span class="text-error">*</span></label>
                            <input type="email" id="manager_email" name="manager_email" value="{{ old('manager_email') }}" required
                                   class="w-full bg-surface/50 border border-outline-variant focus:border-primary focus:ring focus:ring-primary/20 rounded-lg px-4 py-2.5 text-on-surface">
                        </div>
                        <div>
                            <label for="manager_password" class="block font-label-md text-on-surface mb-2">Password Manajer <span class="text-error">*</span></label>
                ...
                <div class="pt-6 border-t border-outline-variant/30">
                    ...
                    @error('address') <p class="text-error font-body-sm mt-1">{{ $message }}</p> @enderror
                </div>
                ...
                <div class="flex justify-end pt-4 border-t border-outline-variant/30 space-x-4">
                    ...
                    <button type="submit" class="px-5 py-2.5 bg-primary hover:opacity-90 text-on-primary rounded-full font-label-md shadow-[0_4px_12px_rgba(0,88,188,0.25)] transition-all flex items-center space-x-2">
                        ...
                        <span>Simpan Cabang & Akun</span>
                    </button>
                </div>
            </form>
        </section>
    @else
        <div class="p-8 bg-error-container text-on-error-container rounded-xl">
